define(['leaflet', 'proj4', 'proj4leaflet', 'leaflet_zoommin', 'leaflet_label', 'jquery', 'pubsub',
       ...
       function(leaflet, proj4, proj4leaflet, leaflet_zoommin, leaflet_label, $, pubsub, _, chroma,
           ...
           var $container = $('.cgr-vis.cgr-vis-choropleth'),
        $tools = $('.cgr-vis-tools'),
        $miniLegend = $('.cgr-vis-legend ul'),
        $fullLegend = $('.cgr-vis-legend-full ul'),
        $fullLegendTrigger = $('.cgr-vis-show-legend-full'),
        $fullLegendBox = $('.cgr-vis-legend-full'),
        $display = $('.cgr-vis-display'),
        $infoTrigger = $('.cgr-vis-show-info'),
        $infoBox = $('.cgr-vis-info'),
        $placeBox = $('.cgr-vis-place'),
        $titleBox = $('.cgr-vis-title'),
        $riskFilter = $tools.find('.cgr-filter-risk').first(),
        $yearFilter = $tools.find('.cgr-filter-year').first(),
        $helpPanel = $('.cgr-vis-help'),
        $toolsPanel = $('.cgr-vis-tools'),
        topics = {
            init: 'init',
    ...
    $('body').on('click', '.cgr-vis-nav-zoom-in', function () {
        map.zoomIn();
        return false;
    });

    $('body').on('click', '.cgr-vis-nav-zoom-out', function () {
        map.zoomOut();
        return false;
    });

    $('body').on('click', '.cgr-vis-nav-zoom-min', function () {
        map.setZoom(1);
        return false;
    });
    ...
    pubsub.subscribe(data.topics.risks, risksHandler);
    ...
    function risksHandler(topic, data) {
        var context = {},
            ext_context = {};
        ...
        _.each(data, function(value) {
            ...
            $riskFilter.append(riskOptionsTmpl(context));
        });
    }
    ...
    function setMapView(topic, data) {
        if (uiState.map.place !== 'undefined' &&
            ...
            // need to zoom out a bit to give better sense of context
    ...
    function redrawDisplay(topic, data) {
        ...
        $infoBox.empty();
        $infoBox.hide();
        // TODO: look up risk title from dataStore.risks and use that rather than risk id ...
        $titleBox.html(titleBoxTmpl({'title': uiState.filter.risk + ' / ' + uiState.filter.year, 'sitename': uiState.embed.sitename}));
    }
    ...
    function initMetaTools() {
        ...
        $yearFilter.on('change', function() {
            ...
            uiState.filter.risk = $riskFilter.val();
            pubsub.publish(topics.tool_change, uiState);
        });
    }
    ...
    function initMetaLegend() {
        ...
        _.each($fullLegendBox.find('.cgr-vis-info-text i'), function(value) {
            $this = $(value);
            score = parseInt($this.data('score'), 10);
            $this.css('background-color', colorScale(score).hex());
        });
    ...
    function getPlaceToolTip(place) {
        ...
        function makeTitle() {
            ...
            if (uiState.filter.risk !== 'all' ||
                typeof(uiState.filter.risk) !== 'undefined') {
                title = title + ' / ' + uiState.filter.risk;
            }
